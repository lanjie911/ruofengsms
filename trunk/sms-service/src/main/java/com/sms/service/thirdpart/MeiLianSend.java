...
public class MeiLianSend {
	...
	public Map<String, Object> sendSmsByMeiLianCMCC(String mobile, String signTip, String content) {
		Map<String,Object> result = new HashMap<>();
		String responseStr = "";
		String encode = "UTF-8";
		String username = prepareParamService.getParam("MEILIAN_NAME_100_CMCC").getParamValue();
		String password_md5 = toMD5(prepareParamService.getParam("MEILIAN_PWD_100_CMCC").getParamValue());
		String apikey = prepareParamService.getParam("MEILIAN_APIKEY_100_CMCC").getParamValue();
		String url = prepareParamService.getParam("meilian_url").getParamValue();
		// 连接超时及读取超时设置
		System.setProperty("sun.net.client.defaultConnectTimeout", "30000"); // 连接超时：30秒
		System.setProperty("sun.net.client.defaultReadTimeout", "30000"); // 读取超时：30秒
		...
		try {
			String smsContent = URLEncoder.encode(signTip + content, encode);	// 对短信内容做Urlencode编码操作。注意：如
			
			// 把发送链接存入buffer中，如连接超时，可能是您服务器不支持域名解析，请将下面连接中的：【m.5c.com.cn】修改为IP：【115.28.23.78】
			buffer.append(url+"?username=" + username + "&password_md5="
					+ password_md5 + "&mobile=" + mobile + "&apikey=" + apikey + "&content=" + smsContent
					+ "&encode=" + encode);
			logger.info("美联账号：{}，请求信息：{}", username, buffer.toString());
			responseStr = httpReq(buffer.toString());
			logger.info("美联账号：{}，响应信息：{}", username, responseStr);
			String[] results = responseStr.split(":");
			if("success".equals(results[0])) {
				result.put("status", true);
				result.put("reqMsgId", results[1]);
	...
	public Map<String, Object> sendSms(Integer accountType, Integer operatorType, String mobile, String signTip, String content) {
		Map<String,Object> result = new HashMap<>();
		String responseStr = "";
		String encode = "UTF-8";
		String username = prepareParamService.getParam("MEILIAN_NAME_"+accountType+"_"+operatorType).getParamValue();
		String password_md5 = prepareParamService.getParam("MEILIAN_PWD_"+accountType+"_"+operatorType).getParamValue();
		String apikey = prepareParamService.getParam("MEILIAN_APIKEY_"+accountType+"_"+operatorType).getParamValue();
		String url = prepareParamService.getParam("meilian_url").getParamValue();
		// 连接超时及读取超时设置
		System.setProperty("sun.net.client.defaultConnectTimeout", "30000"); // 连接超时：30秒
		System.setProperty("sun.net.client.defaultReadTimeout", "30000"); // 读取超时：30秒
