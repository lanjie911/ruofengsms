...
public class FengHuoSend {
	...
	public static void main(String[] args) {
		String username = "rfdxhy";
		String password = "9pkz9v";
		// 构造嗲用接口的客户端
		KyongApiClient client = new KyongApiClient(username, password);
		try {

			// 演示发送下行请求
			String dest = "13373368770";
			String content = "【闪电借款】您的注册验证码为：462889";
			SmsSendStrategy smsSendStrategy = new SmsSendStrategy(dest, content, null, null);
			// 以GET方式发送下行拉取请求
//			System.out.println(client.call(KyongHttpUtils.HttpMethod.GET, smsSendStrategy));
			 ...
			 * 以GET方式状态报告拉取请求
			 * System.out.println(client.call(KyongHttpUtils.HttpMethod.GET,
			 * smsReport)); 以POST方式状态报告拉取请求
			 * System.out.println(client.call(KyongHttpUtils.HttpMethod.POST,
			 * smsReport));
			 */
			...
			*//***
			...
			for (SmsSendResult smsSendResult : resultList) {
				System.out.println(smsSendResult.getMobile() + "----" + smsSendResult.getContent() + "----"
						+ smsSendResult.getResult());
			}*/
		} catch (Exception e) {
			e.printStackTrace();
		}
