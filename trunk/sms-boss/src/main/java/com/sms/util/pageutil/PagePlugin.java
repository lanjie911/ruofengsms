...
public class PagePlugin implements Interceptor {
		...
		if (ivk.getTarget() instanceof RoutingStatementHandler) {
			...
			if (mappedStatement.getId().matches(pageSqlId)) { // 拦截需要分页的SQL
				...
				if (parameterObject == null) {
					...
					AbstractCriteria page = null;
					if (parameterObject instanceof AbstractCriteria) { // 参数就是Page实体
						page = (AbstractCriteria) parameterObject;
	 ...
	 * @return
	 */
	private String generatePageSql(String sql, AbstractCriteria page) {
		if (page != null && dialect != null && dialect.trim().length() > 0 && page.isPaging()) {
